...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		sortPoints(points);
		constructBodyWidthHeight(points);
		constructSkirt(points);
	}

	private void constructBodyWidthHeight(TPoint[] points){
	...
	private void constructSkirt(TPoint[] points){
		...
		for(int i = 0; i < points.length; i++){
			...
			if(points[i].y < minY){
				this.skirt[index] = points[i].y;
				minY = points[i].y;
			}
		}*/

		for(int i = 0; i < points.length; i++){
			if(points[i].x >= 0 && points[i].x < this.skirt.length && points[i].y < this.skirt[points[i].x]){
				this.skirt[points[i].x] = points[i].y;
			}
		}
	}
	...
	*/
	...
	private void swapXY(TPoint[] points){
		//Making every x = -y && every y = x
		for(int i = 0; i < this.body.length; i++){
			points[i] = new TPoint(-this.body[i].y, this.body[i].x);
		}
	}

	private void incrementXs(TPoint[] points){
		//Because x coordinates are nonpositive, we are shifting them
		// by the width of rotated piece (height of initial piece)
		for(int i = 0; i < points.length; i++){
			points[i].x = points[i].x + this.height - 1;
		}
	}

    private void sortPoints(TPoint[] points){
        String[] strPoints = new String[points.length];

        for(int i = 0; i < points.length; i++){
            strPoints[i] = points[i].toString();
        }

        Arrays.sort(strPoints);

        for(int i = 0; i < strPoints.length; i++){
            int x = strPoints[i].charAt(1) - '0';
            int y = strPoints[i].charAt(3) - '0';
            points[i].x = x;
            points[i].y = y;
        }
    }
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while(true){
			...
			if(root.equals(current.computeNextRotation())){
				current.next = root;
				break;
			}
		}
